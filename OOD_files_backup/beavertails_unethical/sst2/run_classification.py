...
def save_results(params_list, freeze_test_set=True):
    ...
    for param_index, params in enumerate(params_list):
        ...
        acc_original, entropy_original, prob_original, ECE_original, cm_original = eval_accuracy(all_label_probs, test_labels, all_train_sentences,
                                                       all_train_labels,
                                                       val_sentences, val_labels, params, lr, epochs, curr_seed)
        acc_calibrated, entropy_calibrated, prob_calibrated, ECE_calibrated, cm_calibrated = eval_accuracy(all_label_probs, test_labels, all_train_sentences,
                                                           all_train_labels,
        ...
        plt.savefig('saved_figures/cm_orig-calibrated_{}_{}.pdf'.format(params['dataset'], params['model']), dpi=600, bbox_inches="tight")
        ...
        plt.savefig('saved_figures/cm_orig-original_{}_{}.pdf'.format(params['dataset'], params['model']), dpi=600, bbox_inches="tight")
...
def eval_accuracy(all_label_probs, test_labels, all_train_sentences, all_train_labels, val_sentences, val_labels,
    ...
    if p_cf is None:
        ...
        # print("\ntrain_sentences: ", all_train_sentences[0:5])
        # print("train_labels: ", all_train_labels[0:5])
        # print("\nval_sentences: ", val_sentences[0:5])
        # print("val_labels: ", val_labels[0:5])
        # clf = DynamicFewShotGPTClassifier(n_examples=1).fit(all_train_sentences, all_train_labels)
        # prompt = clf._get_prompt(val_sentences[0])
        # print("prompt: ", prompt)

        # embed_model = SentenceTransformer('sentence-transformers/all-mpnet-base-v2')
        # train_embeddings = embed_model.encode(all_train_sentences)
        # val_embeddings = embed_model.encode(val_sentences)
        # best_train_sentences = []
        # best_train_labels = []
        # for val_embedding in val_embeddings:
        #     cos_sim = []
        #     for train_embedding in train_embeddings:
        #         cos_sim.append(util.pytorch_cos_sim(val_embedding, train_embedding))
        #     values, indices = torch.topk(torch.Tensor(cos_sim), params['num_shots'])
        #     best_train_sentences.append([all_train_sentences[i] for i in indices])
        #     best_train_labels.append([all_train_labels[i] for i in indices])
        #
        # _, all_val_label_probs = get_model_response_val(params, all_train_sentences, all_train_labels, best_train_sentences, best_train_labels, val_sentences, val_labels, val_sentences, val_labels)
        # # _, all_val_label_probs = get_model_response(params, all_train_sentences, all_train_labels,
        # #                                                 train_sentences, train_labels, val_sentences,
        # #                                                 val_labels, val_sentences, val_labels)
    ...
    return np.mean(correctness_list), entropy_list, prob_list, ECE, cm
