...
def main(models, datasets, all_shots, num_seeds, subsample_test_set, api_num_log_prob, approx, use_saved_results, bs,
    ...
    if use_saved_results:
        ...
        print("\n\n\nepochs:", all_params[0]['epochs'])
        print("lr:", all_params[0]['lr'])
        print("val_size:", all_params[0]['val_size'])
        print("curr_seed:", all_params[0]['val_seed'])
...
def save_results(params_list, freeze_test_set=True):
    ...
    for param_index, params in enumerate(params_list):
        ...
        ### load data
        all_train_sentences, all_train_labels, all_test_sentences, all_test_labels, all_test_labels_actual, all_val_sentences, all_val_labels = load_dataset_custom(
            params)

        print('\n\n all_test_sentences: ', all_test_sentences[0:2])
        ### sample test set
        ...
        print('\n\n test sentences: ', test_sentences[0:2])
        print('\n\n test_sentences length: ', len(test_sentences))
        ...
        # print("\n\ntest_prompts labels: ", type(val_labels[0]))
        # embed_model = SentenceTransformer('sentence-transformers/all-mpnet-base-v2')
        # train_embeddings = embed_model.encode(all_train_sentences)
        # test_embeddings = embed_model.encode(test_sentences)
        # best_train_sentences = []
        # best_train_labels = []
        # for test_embedding in test_embeddings:
        #     cos_sim = []
        #     for train_embedding in train_embeddings:
        #         cos_sim.append(util.dot_score(test_embedding, train_embedding))
        #     values, indices = torch.topk(torch.Tensor(cos_sim), params['num_shots'])
        #     best_train_sentences.append([all_train_sentences[i] for i in indices])
        #     best_train_labels.append([all_train_labels[i] for i in indices])
        # # print("\n\nbest_train_sentences: ", best_train_sentences[0:3])
        ...
        p_cf = get_p_content_free(params, all_train_sentences, all_train_labels, train_sentences, train_labels, val_sentences, val_labels, test_labels,
                                  content_free_inputs=content_free_inputs)  ##type: numpy array e.g. [0.13829783 0.86170214] for SST2

        acc_original, entropy_original, prob_original, ECE_original, cm_original = eval_accuracy(all_label_probs, test_labels, all_test_labels_actual, all_train_sentences,
                                                       all_train_labels,
                                                       val_sentences, val_labels, params, lr, epochs, curr_seed)
        acc_calibrated, entropy_calibrated, prob_calibrated, ECE_calibrated, cm_calibrated = eval_accuracy(all_label_probs, test_labels, all_test_labels_actual, all_train_sentences,
                                                           all_train_labels,
                                                           val_sentences, val_labels, params, lr, epochs,
                                                           curr_seed, mode="diagonal_W",
                                                           p_cf=p_cf)
        accuracies = [acc_original, acc_calibrated]
        ECE = [ECE_original, ECE_calibrated]
        print(f"Accuracies: {accuracies}")
        print(f"Calibration Errors (ECE): {ECE}")
        # print(f"prob_calibrated: {prob_calibrated[0:20]}")
        print(f"p_cf      : {p_cf}")

        plt.figure(figsize=(7.3, 6))
        sns.heatmap(cm_calibrated, 
            annot=True,
            fmt='g', 
        ...
        plt.savefig('saved_figures/cm_calibrated_{}_{}.pdf'.format(params['dataset'], params['model']), dpi=600, bbox_inches="tight")
        ...
        plt.savefig('saved_figures/cm_original_{}_{}.pdf'.format(params['dataset'], params['model']), dpi=600, bbox_inches="tight")

        # print("entropy_original: ", len(entropy_original))
        # print("entropy_original: ", np.array(entropy_original.flatten()))

        # with open('entropy/PT_entropy_{}_{}_{}shots_seed{}.npy'.format(params['model'], params['dataset'], params['num_shots'], params['seed']), 'wb') as f:
        #     np.save(f, np.array(entropy_original.flatten()))
        # with open('entropy/PT_LinC_entropy_{}_{}_{}shots_seed{}.npy'.format(params['model'], params['dataset'], params['num_shots'], params['seed']), 'wb') as f:
        #     np.save(f, np.array(entropy_calibrated.flatten()))

        # def plot_calibration_curve(y_true, y_prob, path, n_bins=10, ax=None, hist=True, normalize=False):
        ...
        #     return curve
        #
        # plot_calibration_curve(test_labels, prob_original, 'entropy/no_calib_{}_{}_{}shots_seed{}.svg'.format(params['model'], params['dataset'],
        #                                                               params['num_shots'], params['seed']))
        # plot_calibration_curve(test_labels, prob_calibrated, 'entropy/with_calib_{}_{}_{}shots_seed{}.svg'.format(params['model'], params['dataset'],
        #                                                               params['num_shots'], params['seed']))


        # fig, ax = plt.subplots(1, 1, figsize=(7, 5.6))
        # bins = np.linspace(0.0, 2.4, 7) - 0.2
        # ax.set_xticks(np.linspace(0.0, 2.4, 7))
        # n, bins, patches = ax.hist([entropy_original.flatten(), entropy_calibrated.flatten()], bins, alpha=0.6,
        #                            weights=[np.ones(len(entropy_original)) / len(entropy_original),
        #                                     np.ones(len(entropy_calibrated)) / len(entropy_calibrated)],
        #                            align='mid')
        # hatches = ['\\\\', '//']
        # colors = ['darkblue', 'red']
        # for patch_set, hatch, color in zip(patches, hatches, colors):
        #     for patch in patch_set.patches:
        #         patch.set_hatch(hatch)
        #         patch.set_edgecolor(color)
        # ax.grid(color='grey', linestyle='dotted', alpha=0.7)
        # ax.set_xlabel('entropy', fontsize=24)
        # ax.set_ylabel('% of total test samples', fontsize=24)
        # ax.set_facecolor("ivory")
        # ax.legend(['GPT', 'Ours'], fontsize=27, loc='upper left')
        # plt.xticks(fontsize=24)
        # plt.yticks(fontsize=24)
        # plt.savefig('entropy/entropy_{}_{}_{}shots_seed{}.svg'.format(params['model'], params['dataset'],
        #                                                               params['num_shots'], params['seed']),
...
def eval_accuracy(all_label_probs, test_labels, all_test_labels_actual, all_train_sentences, all_train_labels, val_sentences, val_labels,
    ...
    if p_cf is None:
        ...
        # sample few-shot training examplesfrom sentence_transformers import SentenceTransformer
    ...
    print('\n\n all_test_labels_actual: ',len(all_test_labels_actual))
    print('all_test_labels_actual: ', all_test_labels_actual[0:10])
    ...
    for label_probs, true_label, in zip(all_label_probs, test_labels):
        ...
        predicted.append(np.array(ans_label))
        ...
        # print('ans_label: ', ans_label.item())
        # print('OOD_actual_label: ', OOD_actual_label)

        # if true_label==2:
        #     OOD_actual_label = all_test_labels_actual[track]
        #     track += 1
        #     if ans_label.item() == 0:
        #         if OOD_actual_label==0:
        #             count_0 += 1
        #         elif OOD_actual_label==1:
        #             count_2 += 1
        #     elif ans_label.item() == 1:
        #         if OOD_actual_label==1:
        #             count_1 += 1
        #         elif OOD_actual_label==0:
        #             count_3 += 1

        if true_label==2:
            OOD_actual_label = all_test_labels_actual[track]
            track += 1
            if ans_label.item() == 0 and OOD_actual_label==0:
                count_0 += 1
            elif ans_label.item() == 0 and OOD_actual_label==1:
                count_2 += 1
            elif ans_label.item() == 1 and OOD_actual_label==1:
                count_1 += 1
            elif ans_label.item() == 1 and OOD_actual_label==0:
                count_3 += 1
            
    entropy_list = np.array(entropy_list)

    print('\n\n Count_00', count_0)
    print('Count_11', count_1)
    print('Count_10', count_2)
    print('Count_01', count_3)

    # print('\n\n calib_prob', calib_prob[0:5])
    ...
    return np.mean(correctness_list), entropy_list, prob_list, 0, cm
